...
export const followUserIds = async (req, res) => {
  try {
    ...
    let following = await Follow.find({ following_user: identityUserId }).select({ followed_user: 1, _id: 0 }).exec();

    // Obtener el array con la información de los usuarios que me siguen a mi (los usuarios que siguen al usuario autenticado)
    let followers = await Follow.find({ followed_user: identityUserId }).select({ following_user: 1, _id: 0 }).exec();
